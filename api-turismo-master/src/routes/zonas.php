<?php
    ...
    //Para armar el carousel CREER de una zona y la galería de imágenes con link a cada localidad 
    $app->get("/zona/{idzona:[0-9]+}/creer/{tipo}", function (Request $request, Response $response, array $args) {
        $buffer = new stdClass();
        $buffer->registros = array();
        //Ciudades de la Zona
        $xSQL = "SELECT zonas_ciudades.idciudad, ciudades.nombre as ciudad FROM zonas_ciudades";
        $xSQL .= " INNER JOIN ciudades ON zonas_ciudades.idciudad = ciudades.id";
        $xSQL .= " WHERE zonas_ciudades.idzona = " . $args["idzona"];
        ...
        while($i < count($ciudades_zona->data["registros"])) {
            $id_ciudad = $ciudades_zona->data["registros"][$i]->idciudad;
            $nombre_ciudad = $ciudades_zona->data["registros"][$i]->ciudad;
            //id de los Atractivos de la Ciudad
            $xSQL = "SELECT * FROM atractivos";
            $xSQL .= " WHERE atractivos.idlocalidad = " . $id_ciudad ;
            $xSQL .= " AND atractivos.tipo = " . $args["tipo"];
            $xSQL .= " ORDER BY atractivos.nombre";
            $atractivos = dbGet($xSQL);
            $i ++;
            $a=0;
            while(count($atractivos->data["registros"]) > $a) {
                //Selecciono un Atractivo al Azar
                $nombre_atractivo = $atractivos->data["registros"][$a]->nombre;
                $descripcion_atractivo = $atractivos->data["registros"][$a]->descripcion;
                $id_atractivo = $atractivos->data["registros"][$a]->id;
                //Selecciono una imagen de ese atractivo al azar
                $xSQL = "SELECT imagen FROM atractivo_imgs WHERE idatractivo = " . $atractivos->data["registros"][$a]->id;
                $imagenes = dbGet($xSQL);
                if(count($imagenes->data["registros"]) > 0) {
                    //$nro = random_int(0, (count($imagenes->data["registros"]) - 1)); //Solo PHP 7^ 
                    $nro = intval(rand(0, (count($imagenes->data["registros"]) - 1))); //Va el -1??
                    array_push($buffer->registros, array(
                        "idlocalidad" => $id_ciudad,
                        "id"=> $id_atractivo,
                        "nombre_localidad" => $nombre_ciudad,
                        "nombre_atractivo" => $nombre_atractivo,
                        "imagen" => $imagenes->data["registros"][$nro]->imagen,
                        "descripcion" =>  $descripcion_atractivo,
                    ));
                } else { //El Atractivo no tiene fotos
                    array_push($buffer->registros, array(
                        "idlocalidad" => $id_ciudad,
