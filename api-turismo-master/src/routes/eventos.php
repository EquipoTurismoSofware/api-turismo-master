...
$app->get("/evento/{id:[0-9]+}", function (Request $request, Response $response, array $args) {
    ...
    if ($respuesta->data["count"] > 0) {
        ...
        if ($color->data["count"] > 0) {
            ...
            $respuesta->data["registros"][0]->color = "722789";
        }
    }
    return $response
        ->withStatus(200)
...
$app->get("/eventos/{cantidad:[0-9]+}", function (Request $request, Response $response, array $args) {
    ...
    $xSQL = "SELECT eventos.*, ciudades.nombre AS localidad FROM eventos";
    $xSQL .= " INNER JOIN ciudades ON eventos.idlocalidad = ciudades.id";
    $xSQL .= " WHERE eventos.hfecha  >= CAST('$hoy' AS datetime)";
    $xSQL .= " ORDER BY eventos.dfecha ASC LIMIT 0, " . $args["cantidad"];
    $respuesta = dbGet($xSQL);
    //Obtener el color si la localidad pertenece a una zona
    if ($respuesta->data["count"] > 0) {
...
//Obtener los últimos X Eventos
$app->get("/eventos/panel/{cantidad:[0-9]+}", function (Request $request, Response $response, array $args) {
    $hoy = date('Y-m-d');
    $xSQL = "SELECT eventos.*, ciudades.nombre AS localidad FROM eventos";
    $xSQL .= " INNER JOIN ciudades ON eventos.idlocalidad = ciudades.id";
    $xSQL .= " WHERE eventos.hfecha  >= CAST('$hoy' AS datetime)";
...
/*Todos los Eventos DESTACADOS */

$app->get("/eventos/destacado/{cantidad:[0-9]+}", function (Request $request, Response $response, array $args) {
    $hoy = date("Y-m-d");
    $xSQL = "SELECT eventos.*, ciudades.nombre AS localidad FROM eventos";
    $xSQL .= " INNER JOIN ciudades ON eventos.idlocalidad = ciudades.id";
    $xSQL .= " WHERE eventos.destacado != 0 AND eventos.hfecha  >= CAST('$hoy' AS datetime)";
    $xSQL .= " ORDER BY eventos.destacado ASC,  eventos.dfecha ASC  LIMIT 0, " . $args[cantidad];
    $respuesta = dbGet($xSQL);
    //Obtener el color si la localidad pertenece a una zona
    if ($respuesta->data["count"] > 0) {
        for ($i = 0; $i <  $respuesta->data["count"]; $i++) {
